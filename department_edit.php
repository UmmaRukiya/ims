...
<body>
        ...
        ***********************************-->
        ...
        <?php 
            $olddata=array();
            $con['id']=$_GET['id'];
            $result=$mysqli->common_select_single('department','*',$con);
            if($result){
                if($result['data']){
                    $olddata=$result['data'];
                }
       ...
       ?>
            ...
            <form method="post" action="">
                <div class="mb-3">
                    <label class="form-label" for="class">Department</label>
                    <input type="text" name="department" class="form-control" id="department" placeholder="department.." value="<?= $olddata->department ?>" />
                </div>
            ...
            <?php 
                if($_POST){
                    ...
                    $_POST['updated_by']=1;
                    $rs=$mysqli->common_update('department',$_POST,$con);
                    if($rs){
                        if($rs['data']){
