...
<body>
        ...
        ***********************************-->
        ...
        <?php 
            $olddata=array();
            $con['id']=$_GET['id'];
            $result=$mysqli->common_select_single('`group`','*',$con);
            if($result){
                if($result['data']){
                    $olddata=$result['data'];
                }
       ...
       ?>
            ...
            <form method="post" action="">
                <div class="mb-3">
                    ...
                    <input type="text" name="`group`" class="form-control" id="`group`" placeholder="Group name.." value="<?= $olddata->group ?>" />
                </div>
            ...
            <?php 
                if($_POST){
                    ...
                    $_POST['updated_by']=1;
                    $rs=$mysqli->common_update('`group`',$_POST,$con);
                    if($rs){
                        if($rs['data']){
